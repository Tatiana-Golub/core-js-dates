...
function getNextFriday(date) {
  ...
  const diff = day - now;
  const e = diff < 1 ? 7 + diff : diff;
  const nextDay = d.getTime() + 1000 * 60 * 60 * 24 * e;
...
function isDateInPeriod(date, period) {
  ...
  if (start <= currentDate && currentDate <= end) return true;
  return false;
}
...
function getNextFridayThe13th(date) {
  do {
    date.setDate(13);
  } while (date.getDay() !== 5 && date.setMonth(date.getMonth() + 1));
  return date;
}
...
function getQuarter(date) {
  const quarterOfYear = Math.ceil((date.getMonth() + 1) / 3);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      workSchedule.push(formatedDate(new Date(currentDate)));
...
function isLeapYear(date) {
  ...
  const leapDate = new Date(currentYear, 1, 29);
  if (leapDate.getMonth() === 1) return true;
  return false;
}
